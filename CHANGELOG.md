...
- 2026-08-03: [auth] | @potensio - Added Host/Origin guard rejecting cross-origin state-changing requests and non-loopback Host headers (DNS rebinding), closing the unauthenticated CSRF hole on POST /api/auth/register; TRUSTED_HOSTS env var allowlists reverse-proxy hostnames (src/web-server.ts)
- 2026-08-03: [auth] | @potensio - Wrapped the whole request handler in one try/catch so any unexpected throw (e.g. SQLITE_BUSY from /api/auth/me, /api/auth/logout, /api/tickets) returns a logged 500 instead of crashing the process (src/web-server.ts)
- 2026-08-03: [auth] | @potensio - Made /api/* default-deny: any non-public API path requires a session before route dispatch, and API paths never fall through to the SPA (unknown /api/ paths now 404 as JSON, not index.html with a 200) (src/web-server.ts)
- 2026-08-03: [auth] | @potensio - Converted hashPassword/verifyPassword to async scrypt so the ~29ms KDF no longer blocks the event loop, hashing before the anyUserExists check in register() to preserve check-then-insert atomicity, plus a 64 KB request body cap (src/auth/password.ts, src/auth/service.ts, src/web-server.ts)
