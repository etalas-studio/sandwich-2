...
- 2026-08-03: [auth] | @potensio - Merged with the pipeline-shape/project-folder-picker/pipeline-run-trigger work: the default-deny guard and CSRF/Host checks now cover every route those pieces added (artifacts, settings/project, run/stop/duplicate/delete/create ticket), none of which had any auth before this merge (src/web-server.ts, src/web-server.test.ts)
...
- 2026-08-04: [ticket-creation] | @riaenriala - Backend ticket creation: migration 0003 (tickets table), repository (createTicket with validation, listTickets), POST /api/tickets and GET /api/tickets routes, registered in web-server. 8 tests (4 repo + 4 route).
- 2026-08-04: [ticket-creation] | @riaenriala - Frontend: Add Ticket button on Tickets page, CreateTicketModal component (form with key/summary/description/url fields), API client (web/src/api/tickets.ts). 6 component tests + wired into TicketsPage with success toast.
