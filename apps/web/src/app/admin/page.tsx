...
export default function AdminPage() {
  ...
  // Toggle to reveal the (stored) API key in a provider's input.
  const [showKey, setShowKey] = useState<Record<string, boolean>>({})
  const toggleKey = useCallback((providerId: string) => {
    setShowKey((prev) => ({ ...prev, [providerId]: !prev[providerId] }))
  }, [])
  const [busy, setBusy] = useState(false)
  // Per-section banner messaging so a failure in one provider doesn't
  // surface confusingly at the top of the page.
  ...
  useEffect(() => {
    if (state.status === 'authenticated' && state.role === 'admin') {
      void load()
    }
  }, [state, load])
    ...
    <main className="min-h-screen bg-neutral-950 px-6 py-10 text-neutral-100">
      <div className="mx-auto max-w-3xl space-y-10">
        <header className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">Admin — AI Engine</h1>
            <p className="mt-1 text-sm text-neutral-500">
              Internal operator panel. Changes apply immediately (no redeploy).
            </p>
          </div>
          <button
...
/** In-section success/error banner (dismissible). */
...
/** Clone a record minus one key (used to clear a section's message). */
