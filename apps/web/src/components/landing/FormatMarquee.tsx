...
export function FormatMarquee({ label }: { label: string }) {
  return (
    <div className="relative z-10 grid grid-cols-12 border-b border-black/10 bg-white/70 backdrop-blur-sm">
      <div className="col-span-12 md:col-span-2 py-6 px-6 md:px-10 border-b md:border-b-0 md:border-r border-black/10 flex items-center">
        ...
        <div className="flex w-max sw-marquee-track">
          {[0, 1].map((rep) => (
            <div key={rep} className="flex items-center shrink-0">
              {FORMATS.map((f) => (
                <div key={f} className="w-40 h-16 flex-shrink-0 flex items-center justify-center border-r border-black/10 opacity-50 hover:opacity-100 transition-opacity">
