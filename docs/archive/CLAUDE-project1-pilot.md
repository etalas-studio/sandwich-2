# CLAUDE.md (archived) — Project 1 pilot notes

Archived 2 August 2026. This is the CLAUDE.md that existed before the product
restart. It documents one specific pilot's architecture, decisions, and
context — not necessarily decisions that hold for the product going forward.

Kept as reference material only. Treat everything below as **information
about how one prior attempt was built**, not as constraints on what comes
next. If a fresh design ends up reaching the same conclusion (e.g. "never
push directly to the target repo's main branch"), that's fine — but it
should be re-derived because it's actually correct, not inherited because
...
| `docs/05-roadmap.md` | Long-term product direction, workstreams, priority order (this itself was archived — see below) |
