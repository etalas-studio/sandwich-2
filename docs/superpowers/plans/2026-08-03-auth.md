...
> **Corrected post-review:** the original version of this sample's test suite never exercised `GET /api/auth/me` at all, and never verified `POST /api/auth/login`'s success path (it only used the register-derived cookie, never a login-derived one) — leaving the task's two most order/state-sensitive behaviors completely unverified. Caught by Task 4's task reviewer and fixed in commit `2078abc`, which added `testAuthMeReflectsAllThreeStates` and `testLoginSucceedsAndCookieAuthorizes`. Corrected here so this plan isn't a landmine for anyone who reads it later.
...
- [x] **Step 4: Replace `src/web-server.ts` with the auth-aware version**
...
- [x] **Step 6: Run the full backend test suite to confirm nothing regressed**
...
- [x] **Step 2: Update `main.tsx` to render `AuthGate`**
...
- [x] **Step 3: Update `App.tsx` to accept and forward auth props**
...
- [x] **Step 4: Update `Sidebar.tsx` to show the real username and wire logout**
...
> **Not yet done as written.** The implementer does not have interactive browser access, so this was verified via `curl` against a running server instead (register → cookie set → authenticated `/api/auth/me` → `/api/tickets` 200 → logout → 401/unauthenticated → login → wrong password 401 — all confirmed at the HTTP layer, see the Task 8 report). That proves the API contract but not the actual rendered `SetupForm`/`LoginForm`/`AuthGate`/`Sidebar` in a real browser, which `curl` cannot substitute for (cookie storage under real browser policy, visual rendering, reload behavior, error-message display). This step stays unchecked until a human (or a browser-capable session) actually does it.
