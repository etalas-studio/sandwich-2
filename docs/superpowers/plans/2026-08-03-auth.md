...
**Interfaces:**
- Consumes: `useAuth` (Task 5), `SetupForm` (Task 6), `LoginForm` (Task 7), existing `App` default export from `web/src/App.tsx` (its props change in Task 9 — until then it takes none, so `AuthGate` renders `<App />` with no props here).
- Produces: `export default function AuthGate()` rendered from `main.tsx` in place of `<App />` directly.
...
Note: this references `App`'s eventual `username`/`onLogout` props, which Task 9 adds. Until Task 9 lands, `npm --prefix web run build` will fail on this file — that's expected and resolved by the next task; do not skip Step 2 below in isolation if executing tasks out of order.
...
- [ ] **Step 3: Commit** (bundled with Task 9 below, since `AuthGate` doesn't typecheck standalone until `App`'s props change — see that task's commit step)

---

### Task 9: Wire logout into `App.tsx` / `Sidebar.tsx`, full manual walkthrough
...
**Interfaces:**
- Consumes: `AuthGate`'s `username: string` / `onLogout: () => void` (Task 8).
- Produces: `App` and `Sidebar` now take `{ username: string; onLogout: () => void }` props — nothing downstream consumes this further.

- [ ] **Step 1: Update `App.tsx` to accept and forward auth props**
...
- [ ] **Step 2: Update `Sidebar.tsx` to show the real username and wire logout**
...
Run: `npm --prefix web run build`
Expected: builds with no TypeScript errors (this is also where Task 8's `AuthGate.tsx` first typechecks cleanly, since `App`'s props now match).
...
Once Task 9 is committed and merged, check off "Auth" in `docs/roadmap.md`'s Phase 1 checklist, linking this plan file, per `CLAUDE.md`'s rule that the roadmap checkbox flips in the same commit as the plan's last task.
